...
static void dump_hex(const char *label, const uint8_t *buf, size_t len)
{
    ...
    for (size_t i = 0; i < len; i += 32) {
        ...
        ESP_LOGI(TAG, "%s [%03u..%03u]: %s",
                 label, (unsigned)i, (unsigned)(i + chunk - 1), line);
    }
}
...
static void init_gpio()
{
    ...
    // INPUT_OUTPUT enables readback on output pins
    ...
    // Pull BUSY and DIO1 low so a floating pin reads 0 rather than random —
    // the SX1262 drives these actively when alive, making pull vs driven unambiguous.
    gpio_set_pull_mode(PIN_LORA_BUSY, GPIO_PULLDOWN_ONLY);
    gpio_set_pull_mode(PIN_LORA_DIO1, GPIO_PULLDOWN_ONLY);

    // MISO left floating for the bias test
    gpio_set_pull_mode(PIN_SPI_MISO, GPIO_FLOATING);
}
...
static esp_err_t spi_bus_up()
{
    ...
    // IOMUX_PINS is an assertion flag: it tells the driver to error out if the
    // given pins are NOT the native SPI2 IOMUX pins (11/12/13 on S3).
    // Actual IOMUX routing happens automatically when native pins are specified.
...
static spi_device_handle_t spi_add_dev(int mode, int hz)
{
    ...
    cfg.mode           = mode;
    cfg.spics_io_num   = -1; // manual CS for maximum visibility
    cfg.queue_size     = 1;
...
static esp_err_t raw_xfer(spi_device_handle_t dev,
                           ...
                           const char *label)
{
    if (!dev || !tx || len == 0 || len > sizeof(s_tx)) return ESP_ERR_INVALID_ARG;
    ...
    if (cs_pin >= 0) cs_low(cs_pin);
    err = spi_device_polling_transmit(dev, &t);
    if (cs_pin >= 0) cs_high(cs_pin);

    spi_device_release_bus(dev);

    if (rx && err == ESP_OK) memcpy(rx, s_rx, len);
...
// ── BME688 ───────────────────────────────────────────────────────────────
// FIX: chip_id register 0xD0 lives on page 0, not page 1.
// Original code set page 1 then read 0xD0, hitting the wrong address.
static bool probe_bme688()
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // Ensure page 0 before reading chip_id (0xD0 is a page-0 register)
        reg_write(dev, PIN_CS_ENV, 0x73, 0x00, true);
        vTaskDelay(pdMS_TO_TICKS(2));
        reg_read_1(dev, PIN_CS_ENV, 0xD0, &chip, true);   // chip_id on page 0
        reg_read_1(dev, PIN_CS_ENV, 0x1D, &status, true); // meas_status_0

        ESP_LOGI(TAG, "BME688 try %d: chip_id=0x%02X status=0x%02X", i + 1, chip, status);
        if (chip == 0x61) ok = true;
    }
...
// ── NEO-M9N ──────────────────────────────────────────────────────────────
// FIX: tx/rx were redeclared in the same function scope — won't compile.
// Split into two explicit scoped blocks.
static bool probe_neo_m9n()
{
    ...
    // 1) Idle poll — NEO-M9N outputs 0xFF when idle and has no data ready
    {
        uint8_t tx[64], rx[64];
        memset(tx, 0xFF, sizeof(tx));
        memset(rx, 0,    sizeof(rx));
        ...
        ESP_LOGI(TAG, "GPS idle poll: zero=%u ff=%u other=%u",
                 (unsigned)zeros, (unsigned)ffs,
                 (unsigned)(sizeof(rx) - zeros - ffs));

        for (size_t i = 0; i + 1 < sizeof(rx); ++i) {
            if (rx[i] == 0xB5 && rx[i + 1] == 0x62) ok = true;
        }
        if (zeros != sizeof(rx) && ffs != sizeof(rx)) ok = true;
    }

    // 2) UBX-MON-VER poll request: B5 62 0A 04 00 00 0E 34
    {
...
// ── SX1262 / E22 ─────────────────────────────────────────────────────────
// FIX 1: tcxo_cmd was declared but never transmitted — BUSY would always timeout.
// FIX 2: send SetDIO3AsTCXOCtrl AFTER BUSY goes low from reset (not before).
//         TCXO must be started before any other command will succeed.
static bool probe_sx1262()
{
    ...
    // Assert reset (min 100µs per SX1262 datasheet; 20ms is more than enough)
    gpio_set_level(PIN_LORA_RST, 0);
    vTaskDelay(pdMS_TO_TICKS(20));
    gpio_set_level(PIN_LORA_RST, 1);

    // Wait for BUSY to go low after reset — chip is in STDBY_RC at this point.
    // Do NOT send any command before BUSY is low.
    int busy_wait = 500;
    while (gpio_get_level(PIN_LORA_BUSY) == 1 && busy_wait-- > 0) {
        vTaskDelay(pdMS_TO_TICKS(1));
    }
    ESP_LOGI(TAG, "SX1262 BUSY after reset: %s (%d ms left)",
             busy_wait > 0 ? "WENT LOW" : "TIMEOUT", busy_wait);

    if (busy_wait > 0) {
        // SetDIO3AsTCXOCtrl: opcode=0x97, tcxo_voltage=1.6V (0x00), timeout=5ms (0x000140)
        // This MUST be the first command. Without it the TCXO never starts and
        // every subsequent command leaves BUSY permanently high.
        uint8_t tcxo_cmd[] = {0x97, 0x00, 0x00, 0x01, 0x40};
        raw_xfer(dev, PIN_CS_LORA, tcxo_cmd, nullptr, sizeof(tcxo_cmd), "sx1262_set_tcxo");

        // Wait for BUSY to clear after TCXO command (~5ms TCXO startup + processing)
        int tcxo_wait = 200;
        while (gpio_get_level(PIN_LORA_BUSY) == 1 && tcxo_wait-- > 0) {
            vTaskDelay(pdMS_TO_TICKS(1));
        }
        ESP_LOGI(TAG, "SX1262 BUSY after TCXO cmd: %s (%d ms left)",
                 tcxo_wait > 0 ? "WENT LOW" : "TIMEOUT", tcxo_wait);
    } else {
    ...
    // GetStatus: opcode=0xC0, one NOP byte. Response: [NOP, status_byte]
    // status_byte bits [6:4]=chip mode, bits [3:1]=cmd status
    for (int i = 0; i < 5; ++i) {
        ...
        if (st != 0x00 && st != 0xFF) ok = true;
        vTaskDelay(pdMS_TO_TICKS(5));
    }

    // ReadRegister syncword 0x0740 as a secondary liveness check
    {
        ...
        if (rx[4] != 0x00 && rx[4] != 0xFF) ok = true;
    }
...
{
    led_init();
    led_set(0, 0, 40); // blue = running

    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "KFDS26 HW_VERIFY");
    ESP_LOGI(TAG, "SPI host=%d hz=%d", SPI_HOST, SPI_HZ);
    ESP_LOGI(TAG, "Pins: MOSI=%d MISO=%d SCLK=%d ENV=%d IMU=%d LORA=%d GPS=%d BUSY=%d DIO1=%d RST=%d LED=%d",
    ...
    led_set(s_any_fail ? 40 : 0, s_any_fail ? 0 : 40, 0); // red=fail, green=pass
